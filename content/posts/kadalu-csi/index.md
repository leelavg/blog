...
I read a couple of blog posts discussing about debugging a (python) application running in a container however they didn't fit my needs well (either they are editor dependent or time taking {{e(i=":confused:")}}).
...
In such cases we need to (docker) commit the container after `cp` of the code blocks, retag and push to the local registry (remember `k3d` cluster can access local registry) and change/edit/patch the image source in yaml manifests. (We'll go through this scenario as well in later posts {{e(i=":smiley:")}})
...
It may seem that we have covered a lot of ground but I had to intentionally drop off some excerpts. I'll be continuing with exploring another component of Kadalu storage in later posts and add any points missed in current post. Stay tuned {{e(i=":eyes:")}}
